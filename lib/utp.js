...
function UTPSocketFactory(socket, handle, timer) {
    ...
    handle._onDestroy = BlockError(function () {
        ...
        socket._closed = true;
        socket._writeCallback(new Error('This socket is closed.'));
        socket._writeCallback = () => {};
    ...
    socket._write = function (chunk, encoding, callback) {
        console.log('write');
        if (socket._closed) callback(new Error('This socket is closed.'));
        else socket._writeCallback = callback;
        else handle.write(chunk, callback);
    };
    socket._writev = function (chunks, callback) {
        console.log('writev');
        var bufList = [], t = 0;
        for (var i = 0; i < chunks.length; i++) {
            bufList.push(chunks[i].chunk);
            t += chunks[i].chunk.length;
        }
        var buf = Buffer.concat(bufList);
        if (socket._closed) callback(new Error('This socket is closed.'));
        else socket._writeCallback = callback;
        else handle.write(buf, callback);
    };
